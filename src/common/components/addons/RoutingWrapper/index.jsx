...
export default class RoutingWrapper extends Component {
 ...
 shouldComponentUpdate (nextProps, nextState) {
   ...
   console.log(Object.keys(nextProps).filter(x => nextProps[x] !== this.props[x]))
 ...
 render () {
   const {routes, services, client} = this.props
   ...
   const routesRendered = onlyRoutes.map((a, i) => {
     ...
     const Tag = a.tag
     const {path, exact, strict, component, render, lazy} = a
     ...
     return <Tag key={i} {...b} render={render} {...{services, client}} />
   })
